...
resultado de la invocación (tip: usá un objeto donde cada propiedad sea el argumento, y 
su valor el resultado de la correspondiente invocación a cb) de manera que, la próxima vez
 que reciba el mismo argumento, no sea necesario volver a invocar a cb, porque el resultado
...
// }
function cacheFunction(cb) {
  const cache = {}; // Objeto para almacenar los resultados

  return function(arg) {
    if (cache[arg] !== undefined) { // Verifica si el resultado ya está en la caché
      return cache[arg];            // Devuelve el resultado almacenado
    } else {
      const result = cb(arg);       // Calcula el resultado llamando a cb
      cache[arg] = result;          // Lo almacena en la caché
